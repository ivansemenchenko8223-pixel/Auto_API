...
from app.schemas.user import UserCreate, UserData
from app.db.session import get_db
from app.servises.user import get_user_by_username, get_user_by_email, get_user_by_phone_number
...
@router.post("/register", status_code=status.HTTP_200_OK, tags=["Аутентификация"], summary="Регистрация пользователя")
def register(user:UserCreate, db:Session=Depends(get_db)):
    ...
    db_user = get_user_by_phone_number(user.phone_number, db)
    if db_user:
